...
public class Person
{
    ...
    private static final String[] APELLIDOS = {"Gutiérrez", "Sánchez", "Abascal", "Feijo", "Ayuso", "Díaz", "Montero"};
    private static final String[] NOMBRESM = {"Pedro", "Santiago", "Juan","Unai", "Álvaro"};
    private static final String[] NOMBRESF = {"Isabel", "Irene", "María","Mercedes"};
    ...
    public Person() {
        Random r = new Random();
        //this.setAge(r.nextInt(MAX_AGE));m
        this.setSurname(APELLIDOS[r.nextInt(APELLIDOS.length)]);
        this.setAge(r.nextInt(MAX_AGE));
        this.setGender(r.nextBoolean());
        if (this.getGender() == GENDER_MALE){
            this.setName(NOMBRESM[r.nextInt(NOMBRESM.length)]);
        } else{
            this.setName(NOMBRESF[r.nextInt(NOMBRESF.length)]);
        }
    }
