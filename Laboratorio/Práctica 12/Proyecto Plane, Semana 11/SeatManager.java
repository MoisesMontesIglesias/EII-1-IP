...
public class SeatManager
{
    ...
    public static final int STANDARD_CLASS = 1;
    ...
    private int maxAge(){
        int max = Person.MIN_AGE;
        for (int i = 0; i<numberOfRows(); i++){
            for (int j = 0; j>numberOfColumns(); j++){
                Person p = getSeat(i,j);
                if(p!= null && p.getAge() > max){
                    max = p.getAge();
                }
            }
        }
    ...
    /**
     * Método childrenPassengers que devuelve los jóvenes que se encuentren dentro del avión
     */
    ...
    /**
     * Método oldestPassengers
     * Devuelve el pasajero de más edad sentado en el avión, si el avión esta vacío devolverá null
     */
    public ArrayList<Person> oldestPassengers(){
        ArrayList<Person> lista = new ArrayList<Person>();
        int max = maxAge();
        for (int i = 0; i<numberOfRows(); i++){
            for (int j = 0; j<numberOfColumns(); j++){
     ...
     * @param int row, fila de asientos del avión
     */
    public int numberOfFreeSeats(int row){
        checkParam(row>0 && row<numberOfRows(), "Fila fuera de rango");
        int a = 0;
    ...
    // /**
     // * Método print que muestra la separación entre asientos de primera y estándar y el hueco del pasillo
     // * Devuelve X en caso de adulto, C en caso de un niño y ? en caso de que el asiento este libre
     // */
    // public void printA()
    // {
        // System.out.print("  ");
    ...
    public void print()
    {
        ...
        for(int i = 0; i<firstRows; i++){
            ...
            for (int j = 0; j<3; j++){
                ...
                if (getSeat(i,j) == null){
                    System.out.print("? ");
                } else if (getSeat(i,j) != null && p.getAge()<p.ADULTHOOD_AGE){
                    System.out.print("C ");
                } else{
                    System.out.print("X ");
                }
        ...
        System.out.print(" -------    -----");
        System.out.println();
        for(int a = firstRows; a<numberOfRows(); a++){
            System.out.print(String.format("%2d ",a));
            for(int b = 3; b<numberOfColumns(); b++){
                Person p = getSeat(a,b);
                if (getSeat(a,b) == null){
                    System.out.print("? ");
                } else if (getSeat(a,b) != null && p.getAge()<p.ADULTHOOD_AGE){
                    System.out.print("C ");
                } else{
                    System.out.print("X ");
                }
    ...
    /**
     * Método getNumPax
     * Devuelve el número de pasajeros sentados en un área del avión
     * El área puede ser de primera clase, clase estándar o todo el avión
     * @param byte area, area del avión(0=primera clase, 1 = clase estándar, 2 = todo el avión)
     */
    public int getNumPax(byte area){
        checkParam(area == 0 || area == 1 || area == 2, "El área está fuera de límites");
        int a = 0;
        if(area == FIRST_CLASS){
            for(int i = 0; i<firstRows; i++){
                for (int j = 0; j<numberOfColumns(); j++){
        ...
        if(area == STANDARD_CLASS){
            for (int h = firstRows; h<numberOfRows(); h++){
                for (int j = 0; j<numberOfColumns(); j++){
                    Person p = getSeat(h,j);
                    if (p != null){
                        a = a+1;
                    }
                } 
    ...
    /**
     * Método getNumPasBySection que devuelve el número de pasajeros sentados en una sección del avión
     * La sección se representa mediante la primera posición de la fila y la última posición de la fila.
     * @param row1, primera fila dada
     * @param colum1, primera columna dada
     * @param row2, segunda fila dada
     * @param colum2, segunda columna dada
     */
    public int getNumPax(int row1, int colum1, int row2, int colum2){
        checkParam(row1 >= 0 && row1 < numberOfRows(), "Fila fuera de límites");
        checkParam(colum1 >= 0 && colum1 < numberOfRows(), "Columna fuera de límites");
        checkParam(row2 >= row1 && row2 < numberOfRows(), "Fila fuera de límites");
        checkParam(colum2 >= colum1 && colum2 < numberOfRows(), "Columna fuera de límites");
        int a = 0;
        for (int i = row1; i<row2+1;i++){
            for (int j = colum1; j<colum2+1; j++){
                Person p = getSeat(i,j);
                if (p!= null){
                    a = a+1;
                }
    ...
    /**
     * Sienta en el avión a un número dado de pasajeros.
     * Se irán sentando en los asientos libres desde las primeras filas del avión
     * @param int paxNumber, número de nuevos ocupantes de asientos del avión
     */
    public void loadPax(int paxNumber){
        checkParam(paxNumber > 0, "Número de nuevos ocupantes fuera de límites");
        ArrayList<Person> lista = new ArrayList<Person>();
        for (int i = 0; i<numberOfRows(); i++){
            for (int j = 0; j<numberOfColumns(); j++){
                Person p = getSeat(i,j);
                if (p != null && paxNumber >= 0){
